...
load_n_trim <- function(filename) {
  raw_counts = read.delim(filename, header=TRUE, sep="\t", row.names='gene')
  subset <- raw_counts[, c("vP0_1","vP0_2","vAd_1","vAd_2")]
...
run_limma <- function(counts_dataframe, design, group) {
  dge <- DGEList(counts = counts_dataframe, group = factor(group))
  ...
  return(topTable(fit, number=Inf, coef=ncol(design), resort.by="P"))
}
...
combine_pval <- function(deseq, edger, limma) {
  a <- tibble(package = 'deseq', pval = deseq$pvalue) %>%
    as.data.frame()
  rownames(a) <- row.names(deseq)
  
  b <- tibble(package = 'edger', pval = edger$PValue)%>%
    as.data.frame()
  rownames(b) <- row.names(edger)
  
  c <- tibble(package = 'limma', pval = limma$P.Value) %>%
    as.data.frame()
  rownames(c) <- row.names(limma)
...
create_facets <- function(deseq, edger, limma) {
  a <- tibble(logFC = deseq$log2FoldChange, padj = deseq$padj, package = 'deseq')
  
  b <- tibble(logFC = edger$logFC, padj = edger$padj, package = 'edger')
  
  c <- tibble(logFC = limma$logFC, padj = limma$adj.P.Val, package = 'limma')
...
theme_plot <- function(volcano_data) {
  ...
  p2 <- volcano_data %>%
    ggplot() +
    geom_point(mapping = aes(x=logFC, y=-log10(padj),color=padj<1e-100)) +
    facet_wrap(~package) +
    theme_light() +
