...
meta_d <- meta_info_from_labels(colnames(filtered_counts[,-1]))
...
library_size_mill <- library_size/10^6
library_size_mill <- pivot_longer(library_size_mill, everything())
library_size_mill$name <- fct_inorder(library_size_mill$name)

ggplot(library_size_mill, aes(x=name,y=value, fill = name)) +
...
cpm_tib <- normalize_by_cpm(filtered_counts)

knitr::kable(head(cpm_tib))
```
...
deseq_tib <- deseq_normalize(filtered_counts, meta_d)

knitr::kable(head(deseq_tib))
...
plot_sample_distributions(filtered_counts[c(-1)], TRUE, "Sample Distributions (Log10(Raw Counts))")
```
...
plot_variance_vs_mean(filtered_counts[c(-1)], TRUE, "Variance vs. Mean (Log10(Raw Counts))")
...
plot_pca(filtered_counts[c(-1)], meta_d, 'Raw Counts PCA')
...
p1<-plot_sample_distributions(cpm_tib, TRUE, "Sample Distribution (log10(CPM))")
p2<-plot_variance_vs_mean(cpm_tib[c(-1)], TRUE, "Variance vs Mean (log10(CPM))")
p3<-plot_pca(cpm_tib, meta_d, 'CPM PCA')
...
p4 <- plot_sample_distributions(deseq_tib[c(-1)], TRUE, "Sample Distribution (log10(DESeq2 norm counts)")
p5 <- plot_variance_vs_mean(deseq_tib[c(-1)], TRUE, "Variance vs Mean (DESeq2)")
p6 <- plot_pca(deseq_tib[c(-1)], meta_d, 'DESeq2 Normalized PCA')
