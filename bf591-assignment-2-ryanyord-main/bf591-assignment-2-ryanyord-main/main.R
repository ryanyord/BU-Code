...
load_expression <- function(filepath) {
  return(as_tibble(read_delim(filepath, delim = ' ')))
}
...
filter_15 <- function(tibble){
  ...
  #creates column for percentage that meet specified criteria and adds to last column
  num_over_15 <- cbind(num_above = rowSums(tibble > (log2(15))))
  
  num_total <- cbind(num_above = rowSums(tibble >= 0))
  
  percent_over_log2_15 <- num_over_15 / num_total
  
  tibble$percentage <- percent_over_log2_15
  
  return_tibble <- filter(tibble, percentage >= 0.15)
  
  return(dplyr::select(return_tibble, -last_col()))
}
...
#' `5           202860_at     DENND4B`
affy_to_hgnc <- function(affy_tibble) {
  affyids <- pull(affy_tibble['probeids'])
  
  ensembl <- useEnsembl(biomart = "genes", dataset = "hsapiens_gene_ensembl")
  
  return(as_tibble(getBM(attributes = c('affy_hg_u133_plus_2', 'hgnc_symbol'),
...
reduce_data <- function(expr_tibble, names_ids, good_genes, bad_genes){
  names_ids <- rename(names_ids, probeids = affy_hg_u133_plus_2)
  
  joined <- inner_join(expr_tibble, names_ids)
  
  joined <- relocate(joined, hgnc_symbol, .after = probeids)
  
  joined <- mutate(joined, gene_set = hgnc_symbol, .after = hgnc_symbol)
  
  good <- filter(joined, gene_set %in% good_genes) %>%
    mutate(gene_set = 'good')
  
  bad <- filter(joined, gene_set %in% bad_genes) %>%
    mutate(gene_set = 'bad')
  
  return_tibble <- bind_rows(good, bad)
...
plot_ggplot <- function(tibble) {
  # Section for ordering in plot.  Adds a column that maps 'good' to 1 and 'bad' to 2
  good_1 <- filter(tibble, gene_set == 'good') %>%
    mutate(gene_set = 1)
  
  bad_2 <- filter(tibble, gene_set == 'bad') %>%
    mutate(gene_set = 2)
  
  bind_1_2 <- bind_rows(good_1, bad_2)
  
  tibble['order'] <- bind_1_2['gene_set']
  
  #plot the boxplot
  p <- tibble %>%
    pivot_longer(cols = 4:38) %>%  # pivot to assess all expression values
    mutate(hgnc_symbol = fct_reorder(hgnc_symbol, order)) %>% # reorder plot display based on good/bad
    ggplot(aes(x=hgnc_symbol, y=value, fill=gene_set)) + 
    geom_boxplot() +
    labs(y= "Expression Levels", x = "Gene", title = "Boxplot of 16 Cancer-related Genes") +
